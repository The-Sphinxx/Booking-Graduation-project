  ...
  <div class="p-6 max-w-5xl mx-auto space-y-8">
    ...
    <div class="flex items-center justify-between border-b pb-6">
      <div class="flex items-center gap-4">
        ...
        <div>
          <h1 class="text-3xl font-bold font-cairo capitalize">{{ config?.title }} Details</h1>
          <p class="text-base-content/60">Viewing details for ID: {{ id }}</p>
        </div>
      </div>
          ...
          <div class="card-body">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-6">
              <div v-for="field in displayFields" :key="field.key" class="border-b border-base-200/50 pb-2 last:border-0 last:pb-0">
                ...
                <!-- Text/Number/Date -->
                <p v-if="['text', 'number', 'tel', 'email', 'date', 'textarea', 'select'].includes(field.type)" class="text-base font-medium">
                  {{ formatValue(data[field.key], field) }}
                </p>

                <!-- Status/Badge -->
                <div v-else-if="field.type === 'status'" class="badge badge-lg" :class="getStatusClass(data[field.key])">
                  {{ data[field.key] }}
                </div>
                
                <!-- Array of strings (Features, etc) -->
                <div v-else-if="Array.isArray(data[field.key]) && typeof data[field.key][0] === 'string'" class="flex flex-wrap gap-2 mt-1">
                   <div v-for="(tag, idx) in data[field.key]" :key="idx" class="badge badge-neutral">{{ tag }}</div>
                </div>

                 <!-- Features Object Array -->
                <div v-else-if="field.key === 'features' && Array.isArray(data[field.key])" class="col-span-2 grid grid-cols-2 sm:grid-cols-3 gap-3 mt-2">
                   <div v-for="(feat, idx) in data[field.key]" :key="idx" class="flex items-center gap-2 text-sm bg-base-200/50 p-2 rounded-lg">
